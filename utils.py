...
def sample_function(user_train, usernum, itemnum, batch_size, maxlen,
    relation_matrix, result_queue, SEED):
...
class WarpSampler(object):
    ...
    def __init__(self, User, usernum, itemnum, relation_matrix, batch_size=
        64, maxlen=10, n_workers=1):
        ...
        for i in range(n_workers):
            self.processors.append(Process(target=sample_function, args=(
                ...
                self.result_queue, np.random.randint(2000000000.0))))
...
def cleanAndsort(User, time_map):
    ...
    User_res = dict()
    for user, items in User_filted.items():
        User_res[user_map[user]] = list(map(lambda x: [item_map[x[0]],
            time_map[x[1]]], items))
    time_max = set()
    for user, items in User_res.items():
        ...
        User_res[user] = list(map(lambda x: [x[0], int(round((x[1] -
            time_min) / time_scale) + 1)], items))
...
def evaluate(model, dataset, args):
    ...
    for u in users:
        ...
        predictions = -model.predict(*[np.array(l) for l in [[u], [seq], [
            time_matrix], item_idx]])
